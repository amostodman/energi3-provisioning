...
case ${REPLY} in
  ...
  a)
    ...
    SQL_REPORT "SELECT DATETIME(updateTime,'unixepoch'),'','U',address,balance,difference,nrgPrice FROM balances;" >> ${RPTTMPFILE}
    ;;
  ...
  b)
    ...
    SQL_REPORT "SELECT DATETIME(updateTime,'unixepoch'),'','U',address,balance,difference,nrgPrice FROM balances WHERE updateTime >= strftime('%s','${PREVMON}-01 00:00:00') and rewardTime <= strftime('%s','${PREVMON}-${LASTDAY} 23:59:59');" >> ${RPTTMPFILE}
  ...
  c)
    ...
    SQL_REPORT "SELECT DATETIME(updateTime,'unixepoch'),'','U',address,balance,difference,nrgPrice FROM balances WHERE updateTime >= strftime('%s','${CURRMON}-01 00:00:00') and rewardTime <= strftime('%s','${CURRMON}-${LASTDAY} 23:59:59');" >> ${RPTTMPFILE}
  ...
  d)
    ...
    SQL_REPORT "SELECT DATETIME(updateTime,'unixepoch'),'','U',address,balance,difference,nrgPrice FROM balances WHERE updateTime >= strftime('%s','${STARTDATE} 00:00:00') and rewardTime <= strftime('%s','${ENDDATE} 23:59:59');" >> ${RPTTMPFILE}
